...
2. ``homography_from_extrinsics`` / ``homography_from_camera`` -- compute it
   analytically from camera intrinsics K + mounting (height, pitch, yaw).
   Convenient in CARLA where these are known exactly. Verify against a real
   frame before trusting it on hardware.
...
def homography_from_points(image_pts, world_pts, grid: GridSpec) -> np.ndarray:
    ...
    # has h33 = 0 (horizon through pixel (0,0)).
...
def homography_from_extrinsics(K, cam_xyz, pitch_deg, yaw_deg, grid: GridSpec) -> np.ndarray:
    ...
    +x). Generalises homography_from_camera for side/rear cameras.
    """
    K = np.asarray(K, dtype=np.float64).reshape(3, 3)
    th = np.radians(pitch_deg)
    yw = np.radians(yaw_deg)

    # Robot (x fwd, y left, z up) -> OpenCV camera axes (x right, y down, z fwd)
...
def homography_from_camera(K, cam_height, pitch_deg, grid: GridSpec) -> np.ndarray:
    """Analytic ground-plane homography from intrinsics + mounting (forward
    camera, yaw=0). Thin wrapper over homography_from_extrinsics."""
...
def bev_known_mask(H, image_shape, grid: GridSpec) -> np.ndarray:
    ...
    the image AND has positive projective depth. A warp-an-all-ones-image
    approach would admit mirror cells BEHIND the camera plane that project
    through the camera centre onto sky pixels (negative w, which
    warpPerspective happily samples) -- e.g. a pitch-0 side camera
    "observing" ground on the opposite side of the vehicle.
    """
    h, w = image_shape[:2]
    try:
