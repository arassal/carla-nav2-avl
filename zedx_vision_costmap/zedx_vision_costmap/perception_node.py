...
class ThreeZedPerceptionNode(Node):
    def __init__(self):
        ...
        self._layer_publishers = {name: self.create_publisher(
            OccupancyGrid, f'/zedx_vision_costmap/layers/{name}', 1)
        ...
        self._bev_pub = self.create_publisher(
            OccupancyGrid, '/zedx_vision_costmap/bev/occupancy', map_qos)
        self._bev_image_pub = self.create_publisher(
            Image, '/zedx_vision_costmap/bev/image', qos_profile_sensor_data)
        self._cloud_pub = self.create_publisher(
            PointCloud2, '/zedx_vision_costmap/points/fused', qos_profile_sensor_data)
        self._status_pub = self.create_publisher(String, '/zedx_vision_costmap/perception_status', 10)
        self._diagnostic_pub = self.create_publisher(
            DiagnosticArray, '/zedx_vision_costmap/diagnostics', 10)
    ...
    def _publish_health(self, level, message, points, started, inflation_radius=0.0,
        ...
        status.level = level
        status.name = 'zedx_vision_costmap/three_zed_perception'
