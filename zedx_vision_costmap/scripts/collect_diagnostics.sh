...
output_dir="${1:-zedx_vision_costmap_diagnostics_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "$output_dir"
...
timeout 5 ros2 topic echo --once /zedx_vision_costmap/perception_status \
  >"$output_dir/perception_status.txt" 2>&1 || true
timeout 5 ros2 topic echo --once /zedx_vision_costmap/diagnostics \
  >"$output_dir/diagnostics.txt" 2>&1 || true
timeout 10 ros2 topic hz /zedx_vision_costmap/costmap \
