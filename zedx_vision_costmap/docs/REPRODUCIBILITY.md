...
```bash
ros2 topic echo --once /zedx_vision_costmap/perception_status
ros2 topic hz /zedx_vision_costmap/bev/occupancy
ros2 topic hz /zedx_vision_costmap/points/fused
...
- `verification.launch.py` published all five layers and the fused map in 0.6;
...
3. Confirm `/zedx_vision_costmap/perception_status` remains `ACTIVE`.
4. Record current/mean/max camera skew and processing latency diagnostics.
5. Inspect `/zedx_vision_costmap/points/fused` in every overlap region. Measure
