...
`config/zedx_vision_costmap.yaml` after calibrating the physical rig. Until then,
double edges, tilted ground, and displaced obstacles are expected and are not a
software synchronization failure.

Acceptance check: display `/zedx_vision_costmap/points/fused` in RViz and inspect
